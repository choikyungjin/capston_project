...
def main(*arguments):
    ...
    for element in file_list_img:
        file_result.append(operate(element))


    try:
        txt_path = "./result2.txt"

        if os.path.exists(txt_path):
            os.remove(txt_path)
        fs = open(txt_path, mode='wt', encoding='utf-8')
        for k in file_result:
            fs.write(k)
            fs.write('\n')
        ...
        raise print('write error')


def operate(_file_name):
    file_name = _file_name
    try:
        image = cv2.imread(global_path + '\\' + file_name)
    ...
    _copy = image.copy()
    init_area = image.shape[0] * image.shape[1] - 3600

    # convert gray and find outline
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    ret, gray = cv2.threshold(gray, 253, 255, cv2.THRESH_BINARY)
    ...
    for i, cct in enumerate(_contours):
        ...
        if largest_area < area < init_area:
            largest_area = area
            largest_index = i
    ...
    kernel = np.float32([[0, -1.0, 0], [-1.0, 5.0, -1.0], [0, -1.0, 0]])
    ...
    # 이름  # cv2.rectangle(_copy2, (40, 50), (120, 90), (0, 255, 0), 2)

    # 예외처리 2, 회전
    first_trim = _copy2[10:60, 50:90]

    #cv2.imshow("trimed red area", first_trim)
    ...
    result = _file_name + '^' + result_name + '^' + result_num
